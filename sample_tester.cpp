...
void CProducerAsync::Start(void) {
    // cout << "asynk producer start" << endl;
    // printf("asynk call\n");
    m_Thr = move(thread(&CProducerAsync::prodThr, this));
    //printf("asynk deliver\n");
}

//-------------------------------------------------------------------------------------------------
void CProducerAsync::Stop(void) {
    // cout << "asynk producer stop" << endl;
...
void CProducerAsync::SendPriceList(unsigned materialID) {
    // cout << "asynk producer send price list" << endl;
    //printf("asynk call\n");
    this_thread::sleep_for(chrono::milliseconds(1000));
    if (materialID == 1) {
...
void CProducerAsync::prodThr(void) {
    // cout << "asynk producer prodthr" << endl;
    ...
    while (true) {
        unique_lock<mutex> locker(m_Mtx);
        m_Cond.wait(locker, [this](void) { return m_Stop || m_Req > 0; });
...
//=================================================================================================
CCustomerTest::CCustomerTest(unsigned count) : m_Count(count) {
    // cout << "created customer" << endl;
}
...
void CCustomerTest::Completed(AOrderList x) {
    // cout << "customer test1 completed" << endl;
    bool mismatch = false;
